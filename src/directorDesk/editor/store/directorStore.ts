...
function deleteObjectIdsFromState(
  state: DirectorRuntimeState,
  objectIds: string[],
  options: { clearSelection?: boolean } = {}
): DirectorRuntimeState {
  const objectIdSet = new Set(objectIds.filter(Boolean));
  if (!objectIdSet.size) return state;

  const removedObjects = state.project.objects.filter((item) => objectIdSet.has(item.id));
  ...
  const nextFocusedCameras = nextCameras.map((camera) =>
    camera.targetObjectId && objectIdSet.has(camera.targetObjectId)
      ? {
          ...camera,
          targetMode: "manual" as const,
          targetObjectId: null,
  ...
  const nextObjects = state.project.objects.filter((item) => !objectIdSet.has(item.id));
  ...
  const selectedObjectIds = state.selectedObjectIds.filter((id) => !objectIdSet.has(id));
  ...
  const selectedCrowdId =
    state.selectedCrowdId && !nextObjects.some((item) => item.kind === "character" && item.crowdId === state.selectedCrowdId)
      ? null
      : state.selectedCrowdId;

  return {
    ...state,
    selectedObjectId: options.clearSelection ? null : selectedObjectId,
    selectedObjectIds: options.clearSelection ? [] : selectedObjectIds,
    selectedCrowdId: options.clearSelection ? null : selectedCrowdId,
    directorInspectorMode: "auto",
    project: {
      ...state.project,
...
export const useDirectorStore = create<DirectorStore>((set, get) => {
  ...
  return {
    ...
    deleteObjects: (ids) =>
      commitMutation((state) => deleteObjectIdsFromState(state, ids)),
    deleteSelectedObject: () =>
      commitMutation((state) => {
        const selectedObjectIds = getOrderedSelectedObjectIds(state);
        if (!selectedObjectIds.length) return state;
        return deleteObjectIdsFromState(state, selectedObjectIds, { clearSelection: true });
      }),
