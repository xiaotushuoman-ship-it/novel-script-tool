...
function isEventInsideNode(event: Event, node: Node | null) {
  ...
  if (eventPath.includes(node)) return true;
...
export function ViewportToolbar({
  ...
  useEffect(() => {
    ...
    function closeMenusOnOutsidePointerDown(event: PointerEvent) {
      if (isEventInsideNode(event, toolbarShellRef.current)) return;
      if (isEventInsideNode(event, toolbarRef.current)) return;
      if (isEventInsideNode(event, characterMenuRef.current)) return;
      if (isEventInsideNode(event, geometryMenuRef.current)) return;
      if (isEventInsideNode(event, crowdPanelRef.current)) return;
      if (isEventInsideNode(event, modelLibraryPanelRef.current)) return;
      if (isEventInsideNode(event, aspectRatioPanelRef.current)) return;
      if (isEventInsideNode(event, sceneLocalModelInputRef.current)) return;
      if (isEventInsideNode(event, libraryLocalModelInputRef.current)) return;
      if (isEventInsideNode(event, panoramaInputRef.current)) return;
  ...
  useLayoutEffect(() => {
    ...
    const frameElement = toolbarElement?.closest(".canvas-frame");
  ...
  return (
    <>
      <div className="viewport-toolbar-shell" ref={toolbarShellRef}>
